...
export function CasinoPortfolio({
  items,
  onActiveProjectChange,
}: CasinoPortfolioProps) {
  // STATE MACHINE
  ...
  const [reshuffledCardIds, setReshuffledCardIds] = useState<Set<string>>(
    new Set(),
  ); // Track cards that have been reshuffled once
  ...
  const {
    addProject: addToShowcase,
    hasProjects,
    selectedProjects,
    clearProjects,
  } = useShowcaseOptional();

  // Audio hook for card sound effects
  const { playSound, playSoundDelayed, isMuted, toggleMute } = useAudio();
  ...
  useEffect(() => {
    ...
    const alreadySaved = filteredItems.filter((item) =>
      savedProjectNames.has(item.common),
    );
    const availableItems = filteredItems.filter(
      (item) => !savedProjectNames.has(item.common),
    );
  ...
  useEffect(() => {
    ...
    const eligibleForReshuffle = rejectedPile.filter(
      (card) => !reshuffledCardIds.has(card.common),
    );
  ...
  const shuffleDeckAnimation = () => {
    ...
    // Play shuffle sound with delay to sync with animation
    playSoundDelayed("shuffle", 400);
    ...
    if (rippleLayerRef.current && containerRef.current) {
      ...
      const children = layer.querySelectorAll(
        ".ripple-wave, .ripple-center-glow, .table-color-shift",
      );
    ...
    const shuffleTl = gsap.timeline({
      onComplete: () => {
        // Cleanup
        if (splitStack.parentNode)
          splitStack.parentNode.removeChild(splitStack);
  ...
  const dealNewHand = () => {
    ...
    const permanentlyRejected = rejectedPile.filter((card) =>
      reshuffledCardIds.has(card.common),
    );
  ...
  const dealCards = (count: number) => {
    ...
    // Play deal sound for each card (staggered)
    for (let i = 0; i < count; i++) {
      setTimeout(() => playSound("deal"), i * 200 + 300);
    }
  ...
  const foldAndInspect = (newIndex: number) => {
    ...
    if (glare)
      gsap.set(glare, { opacity: 0, visibility: "hidden", transition: "none" });
      ...
      if (c) {
        ...
        if (glare)
          gsap.set(glare, {
            ...
            visibility: "hidden",
            transition: "none",
          }); // Kill ALL glare immediately - bypass CSS transition
      }
    });
  ...
  const handleFold = () => {
    ...
    fxLayer.style.cssText =
      "position:absolute;inset:0;pointer-events:none;overflow:visible;z-index:400;";
    ...
    const tl = gsap.timeline({
      onComplete: () => {
        ...
        setTimeout(() => {
          ...
          const validRefs = handRefs.current.filter(
            (c): c is HTMLDivElement => c !== null && c.isConnected,
          );
    ...
    if (tiltInner) {
      tl.to(
        tiltInner,
        { rotateY: 0, rotateX: 0, duration: 0.2, ease: "power2.out" },
        0,
      );
    }
    ...
    tl.to(
      shadow,
      { opacity: 0.7, scale: 1.1, duration: 0.25, ease: "power2.out" },
      0,
    );
    ...
    tl.to(
      card,
      {
        ...
        y: startY - 15,
        boxShadow:
          "0 0 50px rgba(255, 0, 0, 0.7), 0 0 80px rgba(255, 50, 50, 0.4)",
        duration: 0.3,
        ease: "power2.out",
      },
      0,
    ...
    tl.to(
      ...
      {
        boxShadow:
          "0 0 70px rgba(255, 0, 0, 0.85), 0 0 100px rgba(255, 50, 50, 0.5)",
        scale: 0.95,
        duration: 0.12,
        ease: "power2.in",
      },
    ...
    tl.to(
      card,
      {
        ...
        boxShadow:
          "0 0 60px rgba(255, 0, 0, 0.7), 0 0 100px rgba(255, 50, 50, 0.4)",
        duration: 0.2,
        ease: "power2.out",
      },
      0.52,
    ...
    tl.to(
      rejectedPileRef.current,
      {
        scale: 1.18,
        boxShadow:
          "0 0 70px rgba(255, 0, 0, 0.8), 0 0 120px rgba(255, 30, 30, 0.5)",
        duration: 0.08,
        ease: "power4.out",
      },
      0.97,
  ...
  const handleCloseActive = () => {
    ...
    const fxLayer = document.createElement("div");
    fxLayer.style.cssText =
      "position:absolute;inset:0;pointer-events:none;overflow:visible;z-index:200;";
    containerRef.current.appendChild(fxLayer);

    // Helper: Spawn particles of different types at grid position
    const spawnParticle = (
      type: "core" | "spark" | "ember",
      x: number,
      y: number,
    ) => {
    ...
    const glowMid =
      "0 0 50px rgba(255, 0, 255, 0.7), 0 0 100px rgba(255, 0, 255, 0.5), inset 0 0 15px rgba(255, 0, 255, 0.3)";
    const glowHigh =
      "0 0 70px rgba(255, 0, 255, 0.9), 0 0 140px rgba(255, 0, 255, 0.7), inset 0 0 30px rgba(255, 0, 255, 0.5)";
    ...
    tl.to(
      card,
      { scale: 1.04, boxShadow: glowLow, duration: 0.1, ease: "power2.out" },
      0,
    );
    tl.to(
      card,
      { scale: 1.06, boxShadow: glowMid, duration: 0.1, ease: "power2.out" },
      0.1,
    );
    ...
    tl.to(
      card,
      { scale: 1.04, boxShadow: glowLow, duration: 0.08, ease: "power2.in" },
      0.2,
    );
    tl.to(
      card,
      { scale: 1.08, boxShadow: glowMid, duration: 0.08, ease: "power2.out" },
      0.28,
    );
        ...
        for (let row = 0; row < gridSize; row++) {
          for (let col = 0; col < gridSize; col++) {
            ...
            const distFromCenter = Math.sqrt(
              Math.pow(col - 1.5, 2) + Math.pow(row - 1.5, 2),
            );
            ...
            for (let i = 0; i < 2; i++) {
              ...
              setTimeout(
                () => spawnParticle("core", cellX + offsetX, cellY + offsetY),
                cellDelay * 1000,
              );
            }
            ...
            setTimeout(
              () => spawnParticle("spark", cellX, cellY),
              (cellDelay + 0.02) * 1000,
            );
            ...
            if (
              ...
              setTimeout(
                () => spawnParticle("ember", cellX, cellY),
                cellDelay * 1000,
              );
            }
          }
    ...
    tl.call(
      () => {
        [...coreParticles, ...sparkParticles, ...emberParticles].forEach(
          (p, i) => {
            gsap.to(p, {
              opacity: 1,
              scale: 1,
    ...
    tl.call(
      () => {
        ...
        coreParticles.forEach((p, i) => {
          ...
          const midX =
            (startX + targetCenterX) / 2 + Math.cos(angle) * spiralRadius;
          const midY =
            Math.min(startY, targetCenterY) - 60 - Math.random() * 40;
          ...
          gsap.to(p, {
            motionPath: {
              path: [
                ...
                { x: midX, y: midY },
                {
                  x: targetCenterX + (Math.random() - 0.5) * 20,
                  y: targetCenterY,
                },
              ],
              curviness: 1.3,
    ...
    tl.to(
      ghostCard,
      { opacity: 0.8, scale: 1, duration: 0.25, ease: "power2.out" },
      reformStart + 0.1,
    );
    tl.to(
      ghostCard,
    ...
    tl.to(
      interestedPileRef.current,
      { scale: 1.06, duration: 0.08, ease: "power4.out" },
      reformStart + 0.2,
    );
    tl.to(
      interestedPileRef.current,
    ...
    tl.call(
      () => {
        if (interestedPileRef.current) {
          setTimeout(
            () =>
              interestedPileRef.current?.classList.remove(
                "gold-reformation-glow",
              ),
            400,
          );
        }
  ...
  const handleCustomProject = () => {
    ...
    if (focusedIndex !== null) {
      ...
      if (card && containerRef.current) {
        ...
        if (tiltInner)
          gsap.to(tiltInner, { rotateY: 0, duration: 0.3, ease: "power2.out" });
  ...
  const handlePlay = () => {
    ...
    const targetX =
      activeSlotRect.left -
      containerRect.left +
      activeSlotRect.width / 2 -
      cardWidth / 2;
    const targetY =
      activeSlotRect.top -
      containerRect.top +
      activeSlotRect.height / 2 -
      cardHeight / 2;
    ...
    fxLayer.style.cssText =
      "position:absolute;inset:0;pointer-events:none;overflow:visible;z-index:150;";
    containerRef.current.appendChild(fxLayer);
    ...
    trail.style.cssText =
      "position:absolute;left:-100%;top:0;width:200%;height:100%;opacity:0;z-index:-1;";
    ...
    const tl = gsap.timeline({
      onComplete: () => {
        ...
        gsap.to(card, {
          boxShadow:
            "0 0 60px rgba(255, 0, 255, 0.4), 0 0 120px rgba(255, 0, 255, 0.2), 0 20px 60px rgba(0,0,0,0.5)",
          duration: 0.3,
          ease: "power2.out",
        });
      },
    ...
    tl.call(
      () => card.classList.add("gambit-charged", "gambit-vibrating"),
      [],
      0,
    );
  ...
  const handleDraw = () => {
    ...
    setTimeout(() => {
      ...
      tl.to(card, { opacity: 1, duration: 0.1 }).to(card, {
        x: pos.x,
        y: pos.y,
        rotation: pos.rotation,
        scale: 1,
  ...
  return (
    ...
    >
      ...
      <div className="filter-chips-row">
        ...
        <button
          className={cn(
            "filter-chip fullstack",
            activeFilter === "Full Stack Web" && "active",
          )}
        ...
        <button
          className={cn(
            "filter-chip mobile",
            activeFilter === "Mobile Application" && "active",
          )}
        ...
        <button
          className={cn(
            "filter-chip data-science",
            activeFilter === "Machine Learning" && "active",
          )}
        ...
        <button
          className={cn(
            "filter-chip backend",
            activeFilter === "Deep Learning" && "active",
          )}
        ...
        <button
          className={cn(
            "filter-chip n8n",
            activeFilter === "n8n Automation" && "active",
          )}
      ...
      >
        ...
        >
          <div className="text-white/30 font-bold tracking-widest text-xs md:text-lg">
            PROJECTS
          </div>
          <div className="text-white/10 text-3xl md:text-6xl font-black mt-1 md:mt-2">
            {deck.length}
          </div>

          {/* Hover Hint */}
          <div className="absolute bottom-3 md:bottom-6 text-emerald-500/50 text-[10px] md:text-xs uppercase tracking-wider">
            {deck.length > 0 && hand.length < 5
              ? "Click to Draw"
              : deck.length === 0
                ? "Empty"
                : "Hand Full"}
      ...
      >
        ...
        <div
          ...
          style={{
            transform:
              interestedPile.length > 0
                ? `translate(${Math.min(5, interestedPile.length)}px, ${-Math.min(5, interestedPile.length)}px)`
                : "none",
            zIndex: 10,
          }}
        >
          <div className="text-blue-400/80 font-bold tracking-widest text-[8px] md:text-xs">
            SAVED
          </div>
          <div
            className={`text-lg md:text-3xl font-black mt-0.5 md:mt-1 ${interestedPile.length > 0 ? "text-blue-400/60" : "text-white/10"}`}
          >
            {interestedPile.length}
          </div>
        </div>
      </div>
      ...
      >
        ...
        <div
          ...
          style={{
            transform:
              rejectedPile.length > 0
                ? `translate(${Math.min(5, rejectedPile.length)}px, ${-Math.min(5, rejectedPile.length)}px)`
                : "none",
            zIndex: 10,
          }}
        >
          <div className="text-red-400/80 font-bold tracking-widest text-[8px] md:text-xs">
            SKIPPED
          </div>
          <div
            className={`text-lg md:text-3xl font-black mt-0.5 md:mt-1 ${rejectedPile.length > 0 ? "text-red-400/60" : "text-white/10"}`}
          >
            {rejectedPile.length}
          </div>
        </div>
          ...
          <div className="text-white h-full flex flex-col relative">
            ...
            <div className="flex-1 overflow-y-auto pr-1 space-y-3 md:space-y-6 pb-2">
              <div className="pr-8 md:pr-0">
                ...
                <h1 className="text-lg md:text-4xl font-bold mb-1 md:mb-2 tracking-tight leading-tight">
                  {activeProject.common}
                </h1>
                <p className="text-xs md:text-xl text-gray-400 font-light leading-relaxed">
              ...
              <div className="grid grid-cols-2 gap-1.5 md:gap-4">
                {activeProject.features.slice(0, 4).map((f, i) => (
                  <div
                    key={i}
                    className="bg-white/5 p-1.5 md:p-3 rounded-lg border border-white/10 flex items-start hover:bg-white/10 transition-colors"
                  >
                    <Check
                      size={12}
                      className="text-emerald-500 mt-0.5 mr-1 md:mt-1 md:mr-2 flex-shrink-0"
                    />
                    <span className="text-[10px] md:text-sm text-gray-300 line-clamp-2">
                      {f}
                    </span>
                  </div>
                ))}
      ...
      >
        <div className="dealer-chip-inner">
          <div className="dealer-chip-label">
            VIP
            <br />
            DEAL
          </div>
        ...
        <div className="game-over-overlay">
          <div className="game-over-modal">
            ...
            <div className="game-over-stats">
              ...
              <div className="stat-item total">
                <span className="stat-value">
                  {activeFilter
                    ? getFilteredItems(items, activeFilter).length
                    : items.length}
                </span>
            ...
            <div className="game-over-buttons">
              ...
              <button onClick={restartGame} className="game-over-btn restart">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="18"
                  height="18"
